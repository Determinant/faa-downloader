...
export async function buildChartPackages(output = 'dist', regionsFile?: string): Promise<void> {
    ...
    for (const entry of await fs.readdir(root, { withFileTypes: true })) {
        ...
        console.log(`Packaging charts for ${entry.name}`);
        const manifest = await packageChartCycle(directory, path.join(root, entry.name, 'mbtiles'), { regions });
        cycles += 1;
        console.log(`Published ${manifest.archives.length} small archives and ${manifest.regions.length} offline regions`);
    }
...
async function main(): Promise<void> {
    ...
    for (const arg of process.argv.slice(2)) {
        if (arg.startsWith('--output=')) output = arg.slice(9);
            ...
            console.log('Usage: npm run build:chart-packages -- [--output=dist] [--regions=regions.json]\n' +
    ...
    await buildChartPackages(output, regions);
}
