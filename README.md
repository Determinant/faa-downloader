...
| npm run build:terrain | Checks current USGS 3DEP GeoTIFF revisions and downloads changed files, or imports local GeoTIFFs | Publishes 2.45-arc-second integer-metre elevation packages and source provenance at `charts/terrain/` (2.91 GB compressed in the verified national build); independent of chart cycles; run `--estimate` for uncompressed geometry ([details](docs/terrain.md)) |
