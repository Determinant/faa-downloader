...
export function airportFrequencyIndex(airports: CsvRecord[], rows: CsvRecord[]): Map<string, AirportFrequency[]> {
    ...
    for (const row of rows) {
        ...
        if (!type || !/^\d+(?:\.\d+)?$/.test(raw) || frequencyMHz < 100 || frequencyMHz >= 400) continue;
