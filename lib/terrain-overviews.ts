...
export function reduceTerrainArchive(bytes: Buffer): Buffer {
    ...
    for (let quadrant = 0; quadrant < 4; quadrant++) {
        ...
        for (let y = 0; y < 128; y++) for (let x = 0; x < 128; x++) {
            ...
            const maximum = Math.max(grid.readInt16LE(source), grid.readInt16LE(source + 2),
                grid.readInt16LE(source + 512), grid.readInt16LE(source + 514));
            result.writeInt16LE(maximum, ((top + y) * 256 + left + x) * 2);
        }
    }
